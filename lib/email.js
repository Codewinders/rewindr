...
export function offerEmail(fromName, itemTitle, amount) {
  return {
    subject: `Nytt bud på "${itemTitle}": ${amount} kr`,
    html: `
      <div style="font-family: sans-serif;">
        <h2>Nytt bud på Rewindr</h2>
        <p><strong>${fromName}</strong> har lagt ett bud på <strong>${itemTitle}</strong>: <strong>${amount} kr</strong>.</p>
        <p>Logga in på Rewindr för att acceptera, neka eller lägga ett motbud.</p>
      </div>
    `,
  };
}

export function counterOfferEmail(fromName, itemTitle, amount) {
  return {
    subject: `Motbud på "${itemTitle}": ${amount} kr`,
    html: `
      <div style="font-family: sans-serif;">
        <h2>Motbud på Rewindr</h2>
        <p><strong>${fromName}</strong> har lagt ett motbud på <strong>${itemTitle}</strong>: <strong>${amount} kr</strong>.</p>
        <p>Logga in på Rewindr för att acceptera, neka eller lägga ett nytt motbud.</p>
      </div>
    `,
  };
}

export function offerAcceptedEmail(itemTitle, amount) {
  return {
    subject: `Ditt bud på "${itemTitle}" har accepterats!`,
    html: `
      <div style="font-family: sans-serif;">
        <h2>Affär klar på Rewindr</h2>
        <p>Ditt bud på <strong>${itemTitle}</strong> — <strong>${amount} kr</strong> — har accepterats.</p>
        <p>Logga in på Rewindr för att slutföra betalningen.</p>
      </div>
    `,
  };
}
