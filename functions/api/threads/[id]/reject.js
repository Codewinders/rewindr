...
export async function onRequestPost({ request, env, params }) {
  ...
  if (thread.kind !== "trade") return json({ error: "Bara byten kan nekas på det här sättet." }, 400);
  if (thread.owner !== user.username) return json({ error: "Bara ägaren kan neka bytet." }, 403);
  if (thread.status !== "pending") return json({ error: "Bytet är redan hanterat." }, 400);

  await env.DB.prepare("UPDATE threads SET status = 'rejected' WHERE id = ?").bind(params.id).run();
