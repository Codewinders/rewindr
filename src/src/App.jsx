...
function Tabs({ active, setActive, showAdmin, showMyListings }) {
  ...
  return (
    <>
      {/* Mobil: ren ☰-ikon som fäller ut en stapla-lista direkt under */}
      <div className="sm:hidden mb-8">
        <button onClick={() => setMobileOpen((v) => !v)}
          ...
          <Menu size={20} style={{ color: "#ffe94a" }} />
          <span className="flex-1 text-left">{activeLabel}</span>
        </button>
        <div className="overflow-hidden transition-all duration-300"
          style={{ maxHeight: mobileOpen ? tabs.length * 50 + 16 : 0, opacity: mobileOpen ? 1 : 0 }}>
          <div className="flex flex-col gap-1 mt-1.5 p-2 rounded-lg" style={{ background: "#1c1c20", border: "1px solid #33333a66" }}>
            {tabs.map((t) => (
